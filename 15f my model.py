...
### 3.2 Functions used for the models
def modelisation(model=RandomForestClassifier(), df=df_train_num, nb_Fold=15, standardized=False, df_prediction=None):
    ...
    importances = pd.DataFrame(np.zeros((nb_col, nb_Fold)), columns=['Fold_{}'.format(i) for i in range(1, nb_Fold + 1)], index=df.columns)

    #train of the model
    # Creation of the mechanism to split the data
    if nb_Fold > 1:
        #StratifiedKFold: Provides train/test indices to split data in train/test sets.
        skf = StratifiedKFold(n_splits=nb_Fold, random_state=SEED, shuffle=True)
...
def graph_score_model(model=RandomForestClassifier(), nb_Fold=15, step_fold=3, nb_iteration=2, df=df_train_num, standardized=False):
    ...
    folds = np.arange(1,nb_Fold,step_fold)
    list_scores = np.zeros((nb_iteration,nb_Fold // step_fold))
    for j in np.arange(start=0, stop=nb_iteration, step=1):
        for i, fold in enumerate(folds):
            list_scores[j,i] = modelisation(model=RandomForestClassifier(), nb_Fold=fold, df=df, standardized=standardized)[1]
...
modelisation(model=XGBRegressor(), nb_Fold=1)
...
# Predictions
y_pred = modelisation(model=RandomForestClassifier(), df=df_train_dummies, nb_Fold=5, standardized=True, df_prediction=df_test_dummies)

# Graph
modelisation(model=RandomForestClassifier(), nb_Fold=5, df=df_train_num, standardized=False)
